                            ...
                            <table class="table align-middle table-nowrap" id="voucherTable">
                                <thead class="table-light">
                                    <tr class="text--green fs-15">
                                        <th class="fw-medium column-sortable sort {{ $request->sort == 'name' ? $request->sort_direction : ''}}" data-form="#searchForm" data-sort="name" data-direction="{{ $request->sort == 'name' && $request->sort_direction == 'asc' ? 'desc' : 'asc' }}">Name</th>
                                        ...
                                        <th class="fw-medium column-sortable sort {{ $request->sort == 'stayDates' ? $request->sort_direction : ''}}" data-form="#searchForm" data-sort="stayDates" data-direction="{{ $request->sort == 'stayDates' && $request->sort_direction == 'asc' ? 'desc' : 'asc' }}">Logo</th>
                                ...
                                <tbody class="list">
                                    @foreach ($tours as $tour)
                                        <tr class="table__row--medium cursor-pointer text--black fw-normal fs-14" >
                                            <td class="name table__cell--large text-truncate">{{ $tour->name }}</td>
                                            <td class="code">{{ $tour->start_date }}</td>
                                            <td class="code">{{ $tour->end_date }}</td>
                                            <td class="code"></td>
                                            <td>
                                                <div class="d-flex justify-content-center">
                                                    <div>
                                                        <x-button href="{{ route('tour.edit', ['tour' => $tour->id]) }}">
                                                            {{ __('Edit') }}
                                                        </x-button>
                                                        <x-button href="{{ route('tour.delete', ['tour' => $tour->id]) }}">
                                                            {{ __('Delete') }}
                                                        </x-button>
                                                    </div>
                                                </div>
